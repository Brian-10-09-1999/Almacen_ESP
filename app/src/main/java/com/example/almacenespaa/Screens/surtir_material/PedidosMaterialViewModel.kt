...
class PedidosMaterialViewModel (
            ...
            try {
                ...
                if(responseService.piezas2.isNotEmpty()){
                    if (responseService.piezas2[0].surtido=="no1"){
                        ...
                        alertResponse("No se puede surtir porque el componente tiene asociaciones pendientes",context,false)
                    }
            ...
            try {
                ...
                if (responseService.isSuccessful){
                    responseService.body()!!.piezas.forEach {
                        if(it.res!!.toInt()==1){
                            alertResponse("Componente similar asignado correctamente.",context,true)
                        }else{
                            alertResponse("Componente no compatible.\nPor favor Ingresa un componente válido.",context,false)
                        }
                    }
                }
                delay(3000)
    ...
    fun typeSurtido(qrValue:String, context: Context){
        viewModelScope.launch(Dispatchers.IO) {
            try{
                if(Utils(context).getValidationComponent(qrValue)||Utils(context).getValidationComponent2(qrValue)){
